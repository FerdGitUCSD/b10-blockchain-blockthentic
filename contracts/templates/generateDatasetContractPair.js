...
// Load templates
const verificationTemplate = fs.readFileSync(path.join(__dirname, 'DatasetVerificationRegistryTemplate.sol'), 'utf8');
const revocationTemplate = fs.readFileSync(path.join(__dirname, 'RevocationRegistryTemplate.sol'), 'utf8');

// Load placeholders
const verificationPlaceholders = JSON.parse(fs.readFileSync(path.join(__dirname, 'datasetVerificationPlaceholders.json'), 'utf8'));
const revocationPlaceholders = JSON.parse(fs.readFileSync(path.join(__dirname, 'revocationPlaceholders.json'), 'utf8'));

// Map dataset verification categories to revocation categories
...
function generateVerificationContract(options) {
  ...
  // Replace struct
  output = output.replace(/\{\{DATASET_STRUCT\}\}/g, categoryConfig.struct);
  ...
  // Replace get dataset function
  output = output.replace(/\{\{GET_DATASET_FUNCTION\}\}/g, categoryConfig.getDatasetFunction);
...
function generateRevocationContract(options) {
  ...
  // Map dataset verification category to revocation category
  const revocationCategory = categoryMapping[category] || 'datasets';
...
if (require.main === module) {
  ...
  if (args.length < 2) {
    console.log('Usage: node generateDatasetContractPair.js <baseName> <category> [batchEnabled] [issuerName]');
    console.log('');
    console.log('This generates a matched pair of contracts:');
    console.log('  - <baseName>Verification.sol (dataset verification)');
    console.log('  - <baseName>Revocation.sol (revocation registry)');
    console.log('');
    console.log('Categories: research, government, ml, general');
    console.log('');
    console.log('  research   - Academic/scientific datasets with methodology and peer review tracking');
    console.log('  government - Public/government datasets with agency and classification level');
    console.log('  ml         - Machine learning datasets with license and model version tracking');
    console.log('  general    - General-purpose dataset verification');
    console.log('');
    console.log('Examples:');
    console.log('  node generateDatasetContractPair.js NationalLab research true "National Research Lab"');
    console.log('  node generateDatasetContractPair.js CensusData government true "Census Bureau"');
    console.log('  node generateDatasetContractPair.js OpenAIData ml true "OpenAI Research"');
    console.log('  node generateDatasetContractPair.js AcmeData general false "Acme Corp"');
    process.exit(1);
  }
  ...
  try {
    ...
    console.log('');
    console.log('✓ Generated dataset contract pair:');
    console.log('');
