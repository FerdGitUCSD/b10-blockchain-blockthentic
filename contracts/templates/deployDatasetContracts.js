...
async function main() {
  ...
  console.log("\n--- Deploying Dataset Verification Contract Pair ---\n");
  ...
  // Step 2: Deploy DatasetVerificationRegistry
  console.log("2. Deploying DatasetVerificationRegistry...");
  const VerificationRegistry = await hre.ethers.getContractFactory("DatasetVerificationRegistry");
  ...
  console.log("   DatasetVerificationRegistry deployed to:", verificationAddress);
  ...
  console.log("  DatasetVerificationRegistry: ", verificationAddress);
  ...
  if (hre.network.name !== "hardhat" && hre.network.name !== "localhost") {
    ...
    try {
      console.log("Verifying DatasetVerificationRegistry...");
      await hre.run("verify:verify", {
        address: verificationAddress,
        constructorArguments: [revocationAddress, ADMIN_ADDRESS],
      });
      console.log("DatasetVerificationRegistry verified!");
    } catch (error) {
      console.log("DatasetVerificationRegistry verification failed:", error.message);
    }
  }
